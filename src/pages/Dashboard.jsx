...
import SmartInsightsCard from '../components/common/SmartInsightsCard';
import {
  CheckSquare,
  Clock,
  CheckCircle2,
...
export default function Dashboard(props) {
  ...
  const activeTasks = tasks.filter((t) => !t.completed && t.status !== 'Done').length;
  ...
  return (
    <div className="space-y-8 animate-fadeIn font-sans">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-brand-500/20 text-brand-300 border border-brand-500/30 text-xs font-semibold mb-3">
              ...
              <span>Productivity & Workspace Overview</span>
            </div>
            <h2 className="text-2xl sm:text-3xl font-extrabold tracking-tight">
              {greeting}
            </h2>
            <p className="mt-1.5 text-sm sm:text-base text-slate-300 max-w-xl font-normal">
              Plan smarter. Work together. Track daily tasks, engineering deliverables, and project milestones.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        <StatsCard title="Total Tasks" value={totalTasks} total={totalTasks} icon={CheckSquare} color="blue" indicatorText="Total tasks" />
        <StatsCard title="Active Tasks" value={activeTasks} total={totalTasks} icon={Clock} color="amber" indicatorText="In progress" />
        <StatsCard title="Completed" value={completedTasks} total={totalTasks} icon={CheckCircle2} color="emerald" indicatorText="Done" />
        <StatsCard title="High Priority" value={highPriorityTasks} total={totalTasks} icon={AlertOctagon} color="rose" indicatorText="High / Urgent" />
      </div>
